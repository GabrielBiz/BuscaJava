...
public class TTTGraphics2P extends JFrame {
	...
	public TTTGraphics2P() {
		final AtomicReference<EstadoTicTacToe> match = new AtomicReference<EstadoTicTacToe>(new EstadoTicTacToe());
			...
			public void mouseClicked(MouseEvent e) { // mouse-clicked handler
				...
				if (currentState == GameState.PLAYING) {
					if (rowSelected >= 0 && rowSelected < ROWS && colSelected >= 0 && colSelected < COLS
						...
						if (currentState == GameState.PLAYING) {
							EstadoTicTacToe m = match.get();
							m.opponent(rowSelected, colSelected);
							try {
								m = (EstadoTicTacToe) new BuscaCompetitiva(new MostraStatusConsole(), 4).busca(m)
										.getEstado();
							} catch (Exception e1) {
							}
							match.set(m);
							board[m.lastX][m.lastY] = currentPlayer;
							updateGame(currentPlayer, m.lastX, m.lastY);
						}
					}
				} else { // game over
					initGame(); // restart the game
					match.set(new EstadoTicTacToe());
				}
